...
const ModalMainArea = ({ closeModal, children, modalWrapperStyle, modalContainerStyle }) => {
  return (
    <div onClick={closeModal} className={` h-5/6 bg-gray-100 rounded-xl backdrop-filter backdrop-blur bg-opacity-70 fixed left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 overflow-y-scroll ${modalWrapperStyle}`}>
      <div className={`space-y-4 ov-md:p-8 sm:p-6 ${modalContainerStyle}`}>
        {children}
      </div>
    </div>
  )
